...
describe('functions', () => {
  ...
  describe('addNums', () => {
    ...
    it('should add the values passed in the parameters', () => {
      let result = 0;
      const { addNums } = cases;
      result = addNums(2, 2);
      assert.equal(result, 4, 'values: (2, 2) should be 4');

      result = addNums(10, 5);
      assert.equal(result, 15, 'values: (10, 5) should be 15');
    });
  });
  ...
  describe('iterator', () => {
    ...
    it('should call a callback for n times passed to cases.iterator', () => {
      ...
      iterator(9, callBack);
      assert.equal(callBack.callCount, 9);
    });
  });
});
